...
"""オランダ選手 — お立ち台で悲しげにゆっくり話す単独インタビュー"""
...
# 濃いオランダオレンジ一色（contest と同じ）
NETHERLANDS_ORANGE = (0.55, 0.12, 0.0, 1.0)
NETHERLANDS_DARK = (0.28, 0.06, 0.0, 1.0)

# カメラ側（-Y）を向いて話す（yaw=0 が -Y）
NL_YAW = 0.0
NL_POS = Vector((0.0, 0.0, 0.35))  # お立ち台の上
...
def _talk_deltas(frame: int) -> Dict[str, Tuple[float, float, float]]:
    """悲しげにゆっくり話す — 低周波・小さい頷き、やや下向き。"""
    t = frame / FPS
    # レオンより遅い・弱いジェスチャー
    nod = 0.04 * math.sin(t * 3.2) + 0.02 * math.sin(t * 5.1)
    turn = 0.05 * math.sin(t * 1.1 + 0.4) + 0.025 * math.sin(t * 2.3)
    # 溜息っぽい浅い上下
    sigh = 0.035 * math.sin(t * 0.55)
    # 常に少しうつむき気味
    down = 0.12 + 0.04 * math.sin(t * 0.7)
    lean = 0.03 * math.sin(t * 0.9) + sigh
    return {
        "spine_01": (0.04 + lean * 0.5 + down * 0.15, 0.0, turn * 0.2),
        "spine_02": (0.06 + lean + down * 0.25, 0.0, turn * 0.3),
        "neck_01": (0.02 + nod * 0.5 + down * 0.55, 0.0, turn * 0.6),
        "head": (0.03 + nod + down * 0.7, 0.0, turn * 0.85),
    }
...
def _add_nla_loop(
    ...
    strip.repeat = max(
        1.0,
        (frame_end - frame_start + 1) / max(1.0, action.frame_range[1] - action.frame_range[0]),
    )
...
def _hide_pitch_for_studio() -> None:
    ...
    keep_prefixes = ("Light", "Sun", "World", "Camera", "Cam", "Netherlands_", "Interview_")
    for obj in list(bpy.data.objects):
        if obj.type == "CAMERA":
            continue
...
def setup_interview_set() -> None:
    ...
    floor_mat = _mat_rgba("Interview_FloorMat", (0.1, 0.1, 0.11, 1.0), 0.9)
    podium_mat = _mat_rgba("Interview_PodiumMat", (0.16, 0.15, 0.14, 1.0), 0.55)
    back_mat = _mat_rgba("Interview_BackMat", (0.09, 0.07, 0.06, 1.0), 0.85)
    stripe_mat = _mat_rgba("Interview_StripeMat", NETHERLANDS_ORANGE, 0.45)
    accent_mat = _mat_rgba("Interview_AccentMat", NETHERLANDS_DARK, 0.5)
    panel_mat = _mat_rgba("Interview_PanelMat", (0.18, 0.12, 0.06, 1.0), 0.7)
...
def _animate_talking(arm: bpy.types.Object) -> None:
    ...
    # 少し間引きしつつも滑らかに（悲しげはゆっくり）
    for f in range(1, INTERVIEW_FRAMES + 1, 3):
        keys.append((f, _pose_with_deltas(base, _talk_deltas(f), 1.0)))
    if keys[-1][0] != INTERVIEW_FRAMES:
        keys.append(
            (INTERVIEW_FRAMES, _pose_with_deltas(base, _talk_deltas(INTERVIEW_FRAMES), 1.0))
        )
    _add_bone_pose_replace_strip(arm, "Netherlands_SadTalk", 1, INTERVIEW_FRAMES, keys, TALK_BONES)
...
def setup_camera() -> bpy.types.Object:
    _remove_cameras()
    cam_data = bpy.data.cameras.new("CamNetherlandsInterview")
    cam = bpy.data.objects.new("CamNetherlandsInterview", cam_data)
    bpy.context.collection.objects.link(cam)
    bpy.context.scene.camera = cam
    cam_data.lens = 35

    # 正面寄りミディアム、レオンよりさらにゆっくり寄る
    for f in (1, 96, 192, 288, INTERVIEW_FRAMES):
        t = (f - 1) / max(1, INTERVIEW_FRAMES - 1)
        pos = Vector((-0.55 + 0.1 * t, -8.5 + 0.3 * t, 3.35))
        tgt = Vector((0.03 * math.sin(t * math.pi * 0.5), 0.1, 3.0 + 0.08 * t))
...
def _setup_soft_studio_world() -> None:
    ...
    if bg:
        # 少し暖色寄り・暗い（哀しみ）
        bg.inputs["Color"].default_value = (0.07, 0.05, 0.04, 1.0)
        bg.inputs["Strength"].default_value = 0.75


def animate_netherlands_sad_interview() -> None:
    ...
    keys = [(f, NL_POS) for f in range(1, INTERVIEW_FRAMES + 1, 3)]
    if keys[-1][0] != INTERVIEW_FRAMES:
        keys.append((INTERVIEW_FRAMES, NL_POS))
    _animate_root_fixed(root, keys, NL_YAW)

    _add_nla_loop(arm, "idle", 1, INTERVIEW_FRAMES)
    _animate_talking(arm)
    setup_camera()
    scene.frame_set(1)
    print(
        f"Netherlands sad interview: {INTERVIEW_FRAMES}f @ {FPS}fps — "
        "solo somber talk on podium (~16s)"
    )


def render_netherlands_sad_interview_video() -> Path:
    ...
    out = RENDER_DIR / "netherlands_sad_interview.mp4"
    RENDER_DIR.mkdir(parents=True, exist_ok=True)
    scene.render.filepath = str(out)
    print(f"Rendering Netherlands sad interview: {out}")
...
if __name__ == "__main__":
    ...
    if "--render" in sys.argv or "--render-netherlands-sad-interview-video" in sys.argv:
